...
export class UploadProjectAttachmentController
  implements Controller<undefined, UploadFileResponse> {
  constructor(
    @Inject()
  ...
  async handle(request: HttpRequest): HandleResponse<UploadFileResponse> {
    ...
    if (!findedProject) return notFound(new EntityNotFoundError('Project'))
