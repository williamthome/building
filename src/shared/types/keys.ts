export type RequiredLiteralKeys<T> = { [K in keyof T]-?:
  string extends K ? never : number extends K ? never : Record<PropertyKey, any> extends Pick<T, K> ? never : K
} extends { [_ in keyof T]-?: infer U } ? U extends keyof T ? U : never : never

export type OptionalLiteralKeys<T> = { [K in keyof T]-?:
  string extends K ? never : number extends K ? never : Record<PropertyKey, any> extends Pick<T, K> ? K : never
} extends { [_ in keyof T]-?: infer U } ? U extends keyof T ? U : never : never
