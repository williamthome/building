...
type InvertOptionsAndType<O, T> = O extends optional | required | reserved
  ? T
  : optional | required | reserved
...
type SchemaOptions<T, O> = O extends optional | required | reserved
  ? O
  : T extends optional | required | reserved
  ? T
  : never

const schema = <
  TSchema,
  TPickSchema extends Schema<any>,
  KPickSchema extends TPickSchema extends Schema<infer TSchema> ? keyof TSchema : never
>(
  schemas: TSchema,
  pick?: {
    merge: TPickSchema
  ...
  TSchema & Pick<TPickSchema extends Schema<infer TSchema> ? TSchema : never, KPickSchema>
> => {
  if (pick) {
...
const pickSchema = <
  ...
  K extends T extends Schema<infer TSchema> ? keyof TSchema : never
>(
  schema: T,
  keys?: K[]
...
): EmailSchema<SchemaType<string, T, O>, SchemaOptions<T, O>> =>
  new EmailSchema<SchemaType<string, T, O>, SchemaOptions<T, O>>(isEmailValidationOptions)

/// NUMBER ------------------------------------------------

const number = <
  O extends OptionsOrType<number> = required,
  T extends InvertOptionsAndType<O, number> = OptionsResponse<O, number>
>(
  isNumberValidationOptions?: ValidateOptions
): NumberSchema<SchemaType<number, T, O>, SchemaOptions<T, O>> =>
  new NumberSchema<SchemaType<number, T, O>, SchemaOptions<T, O>>(isNumberValidationOptions)

/// BOOLEAN ------------------------------------------------

const boolean = <
  O extends OptionsOrType<boolean> = required,
  T extends InvertOptionsAndType<O, boolean> = OptionsResponse<O, boolean>
>(
  isBooleanValidationOptions?: ValidateOptions
): BooleanSchema<SchemaType<boolean, T, O>, SchemaOptions<T, O>> =>
  new BooleanSchema<SchemaType<boolean, T, O>, SchemaOptions<T, O>>(isBooleanValidationOptions)
...
const custom = <T, O extends optional | required | reserved = required>(): CustomSchema<T, O> =>
  new CustomSchema<T, O>()
