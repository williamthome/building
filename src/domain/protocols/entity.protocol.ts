...
import { ExtractNonReservedPartialSchema, ExtractNonReservedSchema, ExtractSchema, reserved } from './schema/schema.protocol'
...
export type ExtractDto
  <
  T,
  K extends T extends Schema<any> ? keyof Omit<ExtractNonReservedSchema<T>, 'id' | 'companyId'> : never = never
  > = T extends LimitedEntityConstructor<any>
  ? Pick<ExtractNonReservedSchema<T>, Exclude<keyof Omit<ExtractNonReservedSchema<T>, 'id' | 'companyId'>, K>>
...
export type ExtractUpdateDto
  <
  T,
  K extends T extends Schema<infer TSchema> ? keyof Omit<TSchema, 'id' | 'companyId'> : never = never
...
export const entitySchema = <T> (
  schemas: T
): Schema<EntityConstructor<T>> => new Schema<EntityConstructor<T>>({
  id: entityIdSchema,
  ...schemas
})

export const limitedEntitySchema = <T> (
  schemas: T
): Schema<LimitedEntityConstructor<T>> => new Schema<LimitedEntityConstructor<T>>({
  id: entityIdSchema,
