...
class MatchFieldValidation<T> extends Validate<T, MatchFieldValidation<T>> {
  ...
  constructor(private readonly keyToMatch: keyof T, opts: ValidateOptions | undefined) {
    super(opts)
  }
...
export const matchField = <T>(field: keyof T, opts?: ValidateOptions): MatchFieldValidation<T> =>
  new MatchFieldValidation<T>(field, opts)
